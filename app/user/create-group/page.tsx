...
export default function CreateUserGroupPage() {
  ...
  const onSelectionChanged = (event: any) => {
    // Prevent updates during programmatic selection
    if (isUpdatingSelection) {
      return;
    }
    
    const selectedRows = event.api.getSelectedRows();
    // Single selection - only take the first selected row
  ...
  // Sync selected rows when formData changes (for when switching between methods)
  useEffect(() => {
    if (gridApi && formData.step2.selectionMethod === "specific" && !isUpdatingSelection) {
      ...
      if (JSON.stringify(currentSelectedIds.sort()) !== JSON.stringify(formSelectedIds.sort())) {
        ...
        // Reset flag after a short delay to allow the selection change to complete
                        ...
                        <div>
                          ...
                          <ul className="list-disc list-inside mt-1 text-sm">
                            {formData.step2.selectedUsers.map((userId) => {
                              ...
                              return (
                                <li key={userId}>
                                  {user?.displayName || userId} ({user?.email || "N/A"})
                                </li>
                              );
                            })}
                          </ul>
